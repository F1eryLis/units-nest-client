...
import { GetPhoneListsDocument, PhoneList, useUpdatePhoneListMutation } from '../../__generated__/graphql';
...
const EditPhoneModal: React.FC<EditPhoneModalProps> = (({ id, phoneLists, open, onClose }) => {
    ...
    useEffect(() => {
        const phoneList = phoneLists.find(phoneList => phoneList.id === id);
        // const data = phoneListStore.getPhonesListById(id);
        if (phoneList) {
            setPhoneBase(phoneList.name);
            setPhonesList(phoneList.phones);
        }
    }, [id]);
